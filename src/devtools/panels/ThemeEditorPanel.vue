...
<template>
  <section class="bg-[var(--card)] flex flex-col min-h-0 theme-editor">
    <header class="text-[11px] font-semibold text-[var(--muted-foreground)] px-2.5 py-1.5 border-b border-[var(--border)] bg-[var(--secondary)]/30 tracking-wider uppercase">
      <h3 class="m-0 text-[var(--foreground)]">Theme Editor</h3>
    </header>
    <div v-if="isLoading" class="flex-1 overflow-auto p-2 text-[11px] flex flex-col gap-2">
      <span class="text-[var(--muted-foreground)]">Loading…</span>
    </div>
    <div v-else-if="!theme" class="flex-1 overflow-auto p-2 text-[11px] flex flex-col gap-2">
      <span class="text-[var(--destructive)]">{{ error || "No theme loaded" }}</span>
    </div>
    <div v-else class="flex-1 overflow-auto p-2 text-[11px] flex flex-col gap-2">
      <div class="flex items-center gap-2">
        <label class="flex flex-col gap-0.5 text-[10px] text-[var(--muted-foreground)] uppercase">
          Theme
          <select v-model="selectedThemeId" class="dev-select">
      ...
      <fieldset class="border border-[var(--border)] rounded p-1.5 flex flex-col gap-1.5">
        <legend class="text-[var(--primary)] text-[10px] font-semibold px-1 tracking-wider uppercase">Base (objectScale)</legend>
        <div class="grid grid-cols-2 gap-1.5">
          <label class="flex flex-col gap-0.5 text-[10px] text-[var(--muted-foreground)] uppercase">
            widthRatio
            <Input type="number" step="0.01" class="h-7 font-mono text-[11px]"
      ...
      <fieldset class="border border-[var(--border)] rounded p-1.5 flex flex-col gap-1.5">
        <legend class="text-[var(--primary)] text-[10px] font-semibold px-1 tracking-wider uppercase">Per-file (fileScales / fileOffsets)</legend>
        <label class="flex flex-col gap-0.5 text-[10px] text-[var(--muted-foreground)] uppercase col-span-full">
          File
          <select v-model="selectedFile" class="dev-select">
        ...
        <div class="grid grid-cols-3 gap-1.5">
          <label class="flex flex-col gap-0.5 text-[10px] text-[var(--muted-foreground)] uppercase">
            scale
            <Input type="number" step="0.01" class="h-7 font-mono text-[11px]" placeholder="(default 1.0)"
              :model-value="fileScale ?? ''"
              @input="setFileScale(+($event.target as HTMLInputElement).value)" />
          </label>
          <label class="flex flex-col gap-0.5 text-[10px] text-[var(--muted-foreground)] uppercase">
            offsetX
            <Input type="number" step="1" placeholder="0" class="h-7 font-mono text-[11px]"
        ...
        <p class="text-[var(--muted-foreground)] text-[10px] m-0">fileScales/fileOffsets keys match the SVG/APNG filename. Empty values delete the override.</p>
      </fieldset>

      <p v-if="error" class="text-[10px] m-0 p-1 rounded bg-[var(--destructive)] text-destructive-foreground">
        {{ error }}
      </p>
      <p v-if="saveBanner" class="text-[10px] m-0 p-1 rounded bg-[var(--primary)] text-[var(--primary-foreground)]">
        {{ saveBanner }}
      </p>
    </div>
  </section>
