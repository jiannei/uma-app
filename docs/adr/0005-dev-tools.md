...
- **前端**：
  ...
  - 新增 1 个共享 JS 模块：把 `state-machine.js` 抽到 dev 窗口也能 import（**已在 ADR-0006 替换为 `pet-machine.ts` + `@xstate/vue`，两个 webview 改用 `useMachine(petMachine)` 拿到同一份 actor 定义**）
  - **主设置窗口无任何 dev 相关 UI**（override 后的简化）

### 长期影响

- **两个 DisplayStateResolver 实例同时运行**（pet 的 + dev panel 的；详见 ADR-0006）。两者都是 `useMachine(petMachine, { input: { theme } })` 的同一种 actor——同一 machine 定义、同一 Vue composable——所以**行为必然一致**（排除 React/Vue render cycle 等运行时偶然差异）。如果 sprite 和 Panel 1 显示不同状态，那是真 bug，不是"独立计算"漂移。
